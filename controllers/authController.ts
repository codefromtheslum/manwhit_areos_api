...
import { sendVerification } from "../config/emailServices";
...
export const createPassword: any = async (req: Request, res: Response) => {
  try {
    ...
    const hashedPassword: any = bcryptjs.hash(password, 10);
...
export const loginAccount: any = async (req: Request, res: Response) => {
  try {
    ...
    if (!user) {
      return res.status(400).json({
        message: `The email you've entered does not exist, create an account now`,
      });
    }
    ...
    return res.status(500).json({
      message: `Error occured during account login`,
      data: error?.message,
    });
  }
};
